...
namespace Urho3D {
    ...
    void Constraint::DrawDebugGeometry(DebugRenderer* debug, bool depthTest)
    {
        ...
        Color xAxisC;
        xAxisC.FromHSL(Wrap(Color::RED.Hue() + hueOffset, 0.0f, 1.0f), Color::RED.SaturationHSL(), Color::RED.Lightness());
        Color yAxisC;
        yAxisC.FromHSL(Wrap(Color::GREEN.Hue() + hueOffset, 0.0f, 1.0f), Color::GREEN.SaturationHSL(), Color::GREEN.Lightness());
        Color zAxisC;
        zAxisC.FromHSL(Wrap(Color::BLUE.Hue() + hueOffset, 0.0f, 1.0f), Color::BLUE.SaturationHSL(), Color::BLUE.Lightness());
    ...
    Urho3D::Matrix3x4 Constraint::GetOwnWorldFrame() const
    {
        ...
        Matrix3x4 worldFrame = ownBody_->GetNode()->LocalToWorld(Matrix3x4(position_, rotation_, 1.0f));
    ...
    Urho3D::Matrix3x4 Constraint::GetOtherWorldFrame() const
    {
        if (otherBody_) {
            ...
            //return a frame with no scale at the position and rotation in node space.
            Matrix3x4 worldFrame = otherBody_->GetNode()->LocalToWorld(Matrix3x4(otherPosition_, otherRotation_, 1.0f));
