...
namespace Urho3D {
    ...
    void Urho3D::HingeConstraint::SetFriction(float friction)
    {
        ...
        if (frictionTorque_ != friction) {
            ...
            if (newtonJoint_) {
                if (powerMode_ == NO_POWER)
                    dynamic_cast<dCustomHinge*>(newtonJoint_)->SetFriction(physicsWorld_->SceneToPhysics_Domain(frictionTorque_));
            }
            else
                MarkDirty();
        }
    ...
    void Urho3D::HingeConstraint::SetMaxTorque(float torque)
    {
        if (maxTorque_ != torque)
        {
            ...
            {
                if (powerMode_ == ACTUATOR)
                    static_cast<dCustomHingeActuator*>(newtonJoint_)->SetMaxTorque(physicsWorld_->SceneToPhysics_Domain(maxTorque_));
                else if (powerMode_ == MOTOR)
                    static_cast<dCustomHinge*>(newtonJoint_)->SetFriction(physicsWorld_->SceneToPhysics_Domain(maxTorque_));
            }
            else
                MarkDirty();
        }
